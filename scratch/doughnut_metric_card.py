      ...
      <div className={`relative ${dim}`}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="text-gray-400 mb-0.5">
            {icon && React.cloneElement(icon, { size: isSm ? 12 : 18 })}
          </div>
          <span className={`${isSm ? 'text-lg' : 'text-3xl'} font-bold text-gray-900 dark:text-white leading-none`}>
            {value}
          </span>
          <span className={`${isSm ? 'text-[8px]' : 'text-[10px]'} font-bold text-gray-400 uppercase tracking-widest mt-1`}>
            {label}
          </span>
        </div>
      </div>
...
start_marker = "function MetricCard({ label, value, max = 240, icon, color, active, size = 'md' }) {"
# Fallback to previous marker if not found
...
if start_idx != -1:
    ...
    if end_idx != -1:
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("MetricCard replaced with Doughnut Chart successfully.")
    else:
