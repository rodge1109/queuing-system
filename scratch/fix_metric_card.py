...
# Search for all occurrences of function MetricCard and the messy stuff
import re
pattern = r'function MetricCard.*?\}\n\}' # Attempt to catch the double curly brace mess
content = re.sub(r'function MetricCard\(.*?\)\s+\{.*?\}\n\)\s+\{.*?\}', new_metric_card, content, flags=re.DOTALL)
# Also catch the other possible mess
content = re.sub(r'function MetricCard\(.*?\)\s+\{.*?\}', new_metric_card, content, flags=re.DOTALL)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("MetricCard cleaned and updated.")
